                    ...
                    <div className="relative">
                      {post.images.length > 1 ? (
                        <div className="flex items-center gap-4">
                          <button
                            onClick={() => {
                              const currentIndex = post.images.indexOf(post.images[0]);
                              const prevIndex = currentIndex > 0 ? currentIndex - 1 : post.images.length - 1;
                              post.images[0] = post.images[prevIndex];
                              setPosts([...posts]);
                            }}
                          ...
                          <img
                            src={post.images[0]}
                            alt={`Пост ${post.id} изображение`}
                            className="w-full max-h-96 object-cover rounded-lg cursor-pointer"
                            onClick={() => handleImageClick(post.images[0], allPhotos.indexOf(post.images[0]))}
                            role="button"
                            aria-label={`Открыть изображение поста ${post.id}`}
                            tabIndex={0}
                            onKeyDown={(e) => e.key === 'Enter' && handleImageClick(post.images[0], allPhotos.indexOf(post.images[0]))}
                          />
                          <button
                            onClick={() => {
                              const currentIndex = post.images.indexOf(post.images[0]);
                              const nextIndex = currentIndex < post.images.length - 1 ? currentIndex + 1 : 0;
                              post.images[0] = post.images[nextIndex];
                              setPosts([...posts]);
                            }}
